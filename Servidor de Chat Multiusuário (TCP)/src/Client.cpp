...
Client::~Client() {
    // A chamada principal para fechar o socket foi movida para a função start()
    // para resolver o deadlock. Manter a limpeza do WSA aqui é uma boa prática.
#ifdef _WIN32
    WSACleanup();
#endif
}
...
void Client::start() {
    ...
    // O programa fica aqui até o usuário digitar /quit
    sendLoop();

    // --- CORREÇÃO DO DEADLOCK ---
    // Após o usuário digitar /quit, feche o socket.
    // Isso fará com que o recv() na receiveLoop falhe e a thread termine.
    CLOSESOCKET(sock);
    // --- FIM DA CORREÇÃO ---

    // Agora o join() vai ser desbloqueado e o programa pode encerrar corretamente.
    recv_thread.join();
}
